//This macro is re-analysis of what has been already done
//
...
//Splitting channels to prepare for background subtraction
selectWindow(title);
run("Split Channels");

//Closing out uncessary channels 
...
waitForUser("Click when Asyn is de-noised"); 
close("Asyn");
selectWindow("TUJ1-copy");
run("PureDenoise ...");
...
rename("Asyn");
run("8-bit");
selectWindow("Denoised-TUJ1-copy");
rename("TUJ1-copy");
run("8-bit");
selectWindow("Denoised-Macrophage");
rename("Macrophage");
run("8-bit");
selectWindow("Macrophage");

//Threshold for total quantification and overlap (not the ganglia specific ones)
		
//****************************************************THRESHOLD STUFF below
//Beginning the Thresholding Process for TUJ1/Asyn images (Phil said no stack)
selectWindow("Asyn");
