...
//QUESTION: TO LEAVE DE-NOISING IN? i THINK not BUT 
...
//		
			for(a=1; a < roi_count + 1; a++) {
				...
				run("Duplicate...", "duplicate");
				rename("TJ" + a);
				run("Merge Channels...", "c1=[Denoised-C2-ROI" + a + "], c2=[Denoised-C3-ROI" + a + "] create keep");
			}
//spaceholder
			...
			for(b=1; b < roi_count + 1; b++) {
				...
				selectWindow("Denoised-C3-ROI" + b - 0);
				imageCalculator("AND create stack", "Denoised-C3-ROI" + b - 0,"Denoised-C2-ROI" + b);
			...
			for (n=1; n < roi_count + 1; n++) {
				...
				close();
				selectWindow("Denoised-C3-ROI" + n + "-1");
				close();
				/*selectWindow("C1-ROI" + n);
				close();
				selectWindow("C2-ROI" + n);
